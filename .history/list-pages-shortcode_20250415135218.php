...
function list_pages_shortcode_enqueue_scripts() {
    wp_enqueue_style('list-pages-dropdown', plugins_url('css/dropdown.css', __FILE__));
    wp_enqueue_script('list-pages-dropdown', plugins_url('js/dropdown-new.js', __FILE__), array('jquery'), '1.0.0', true);
}
...
class List_Pages_Shortcode_Walker_Page extends Walker_Page {
	...
	public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
        ...
        // Add dropdown toggle functionality for items with children
        if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
		...
		if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
			$item .= '<span class="dropdown-toggle-icon" data-page-id="' . $page->ID . '">+</span>';
		}
