...
class List_Pages_Shortcode_Walker_Page extends Walker_Page {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		// Allow up to 3 levels by adjusting the depth restriction
		if ( isset( $args['depth'] ) && $depth >= $args['depth'] - 1 ) {
			return; // Stop rendering levels deeper than the specified depth
		}
		$indent    = str_repeat( "\t", $depth );
		$list_type = List_Pages_Shortcode::validate_list_type( $args['list_type'] );
		$output   .= "\n$indent<" . $list_type . " class='children dropdown-menu'>\n";
	}
	...
	public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		// Allow up to 3 levels by adjusting the depth restriction
		if ( isset( $args['depth'] ) && $depth >= $args['depth'] ) {
			return; // Stop rendering elements deeper than the specified depth
		}
		...
		if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
			// Only show the toggle button if we're not at the maximum depth level
			if ( !isset($args['depth']) || $depth < ($args['depth'] - 1) ) {
				$item .= '<button class="dropdown-toggle" aria-expanded="false" aria-label="Toggle submenu for ' . esc_attr( $page->post_title ) . '" onclick="toggleDropdown(this)">+</button>';
			}
		}
		...
		$output .= $indent . '<li class="' . esc_attr( $css_class ) . '" data-has-children="' . ( isset( $args['pages_with_children'][ $page->ID ] ) ? 'true' : 'false' ) . '">' . apply_filters( 'list_pages_shortcode_item', $item, $page, $depth, $args, $current_page );
	}
 ...
 */
function list_pages_shortcode_enqueue_scripts() {
    wp_enqueue_script(
        'list-pages-shortcode-js',
        plugins_url( 'list-pages-shortcode.js', __FILE__ ),
        array( 'jquery' ),
        '1.0.0',
        true
    );
}
add_action( 'wp_enqueue_scripts', 'list_pages_shortcode_enqueue_scripts' );

/**
 * Enqueue styles for the plugin.
 */
function list_pages_shortcode_enqueue_styles() {
    wp_enqueue_style(
        'list-pages-shortcode-css',
        plugins_url( 'list-pages-shortcode.css', __FILE__ ),
        array(),
        '1.0.0'
    );
}
add_action( 'wp_enqueue_scripts', 'list_pages_shortcode_enqueue_styles' );
